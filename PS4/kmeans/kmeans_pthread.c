...
int updated = 1; //Variable that says if a point was updated in an iteration

Point* points;       // Array containig all points
Centroid* centroids; // Array containing all centroids

int counter = 0; //counter to be used with barrier
...
void* reassign_points(void* rank) {
    //Reassign points to closest centroid
    long my_rank = (long)rank;
    int startPoint = my_rank * nPoints/nThreads;
    int endPoint = (my_rank + 1) * (nPoints/nThreads) - 1;   
    ...
    for(int i = startPoint; i <= endPoint; i++){
        ...
        if(bestCluster != points[i].cluster){
            updated = 1; //This isnt a critical point because it doesnt matter if two threads set it to the same value at the same time
        }
        points[i].cluster = bestCluster;
    }
}

void* compute_centroids(void* rank) {
...
int main(int argc, char** argv){
    ...
    while(updated){
        ...
        for (t = 0; t < nThreads; t++) {
            pthread_create(&thread_handles[t], NULL, reassign_points, (void*)t);
        }
        for (t = 0; t < nThreads; t++) {
